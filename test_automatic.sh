...
rm -f "${OUTPUT_DIR}"/automatic_segment_*.png
...
curl -X POST http://localhost:8000/runsync \
     ...
     -d @test_input_automatic.json > "$TEMP_RESP"
...
while IFS= read -r line; do
    if [[ $line == *"DEBUG"* && $line == *"image_base64"* ]]; then
        ...
        if [[ $line =~ image_base64\":\"([^\"]+) ]]; then
            ...
            output_file="${OUTPUT_DIR}/automatic_segment_${counter}.png"
...
ls -l "${OUTPUT_DIR}"/automatic_segment_*.png 